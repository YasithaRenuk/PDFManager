...
const PDFViewer = () => {
  ...
  useEffect(() => {
    const fetchPDF = async () => {
      try {
        if (user && user.token) {
          const response = await axios.get(`http://localhost:8070/PDFmanagement/download/${id}`, {
            headers: {
  ...
  return (
    <Flex direction="column" height="100vh">
      <Box flex="1" position="relative">
        <iframe src={pdfURL} title="PDF Viewer" width="100%" height="100%" style={{ border: 'none' }} />
      </Box>
      <Box p="4" bg="gray.100" textAlign="center">
        <Button colorScheme="blue" onClick={handleDownload} isDisabled={!pdfURL}>
          Download
        </Button>
      </Box>
    </Flex>
